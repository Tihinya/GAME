import { isEvent, isProps, propertyHasChanged } from "../utils/filters"
...
function updateDomNode(
	...
	const oldPropsArr = Object.entries(oldProps || {})
		.filter(([key]) => isProps(key))
		.filter(([key, value]) => propertyHasChanged(key, value, newProps))
	const newPropsArr = Object.entries(newProps || {})
		.filter(([key]) => isProps(key))
		.filter(([key, value]) => propertyHasChanged(key, value, oldProps))
